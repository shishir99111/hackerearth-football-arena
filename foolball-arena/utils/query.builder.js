/**
 * Custom Made Library for generating query handling special cases of pagination
 */
...
QueryBuilder.prototype.like = function (column, value) {
  this._mainQuery.push(`${column} like ${this._pushMainParam(value)}`);
  if (this.buildTotalQuery) this._totalQuery.push(`${column} like ${this._pushTotalParam(value)}`);
  return this;
};
...
QueryBuilder.prototype.paginateQuery = async function (mysql) {
  this.mainQuery = this._mainQuery.join(' ');
  this.totalQuery = this._totalQuery.join(' ');
  const totalRecords = await mysql.query(this.totalQuery, this.totalValues);
  const filteredRecords = await mysql.query(this.mainQuery, this.mainValues);
  // totalRecords = totalRecords[0].count;
  // filteredRecords = filteredRecords;
  return this.formatPaginationResponse(filteredRecords, totalRecords);
};

QueryBuilder.prototype.findOne = async function (mysql) {
  this.mainQuery = this._mainQuery.join(' ');
  let record = await mysql.query(this.mainQuery, this.mainValues);
  record = record.rows;
  if (record.length > 1) {
...
QueryBuilder.prototype.query = async function (mysql) {
  this.mainQuery = this._mainQuery.join(' ');
  const record = await mysql.query(this.mainQuery, this.mainValues);
  return record;
};
